...
module.exports.settings = function (cwd) {
  var createConfig = function (name, args) {
    var localPath = path.join(cwd, 'node_modules', '.bin', 'grunt');
    var exec = fs.existsSync(localPath) ? localPath : 'grunt';
    return {
      name: name,
      exec: exec,
      sh: false,
  ...
  return new Promise(function(resolve, reject) {
    ...
    Task.once(require.resolve('./grunt-parser-task.js'), cwd, function (result) {
      ...
      result.tasks.sort(function (t1, t2) {
        return ('default' === t1) ? Number.NEGATIVE_INFINITY : t1.localeCompare(t2);
      });
