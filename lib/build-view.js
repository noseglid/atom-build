...
export default class BuildView extends View {
  ...
  resizeMoved(ev) {
    ...
    switch (atom.config.get('build.panelOrientation')) {
      case 'Bottom': {
        ...
        cssKey = 'height';
        size = Math.min(maxHeight, nearestRowHeight);
        break;
      }
      ...
      case 'Left': {
        ...
        cssKey = 'width';
        size = this.terminalEl.width() + delta;
        break;
      }
    ...
    this.terminalEl.css(cssKey, `${size}px`);
    this.resizeTerminal();
  }
  ...
  orientationFromConfig() {
    ...
    $('.build .panel-heading, .build .panel-footer').css('cursor', 'auto');

    switch (atom.config.get('build.panelOrientation')) {
      case 'Bottom':
        this.panelHeading.get(0).addEventListener('mousedown', this.resizeStarted);
        this.panelHeading.css('cursor', 'row-resize');
        break;

      case 'Top':
        this.resizer.get(0).addEventListener('mousedown', this.resizeStarted);
        this.resizer.css('cursor', 'row-resize');
        break;

      case 'Left':
      case 'Right':
        this.resizer.get(0).addEventListener('mousedown', this.resizeStarted);
        this.resizer.css('cursor', 'col-resize');
        break;
    }
  }
