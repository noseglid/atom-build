...
module.exports = {
  ...
  activate: function(state) {
    ...
    this.errorMatcher.on('parsed', function (matches) {
      _.each(matches, function (match) {
        match.length = match[0].length; // needed by insertLinks
      });
      this.buildView.insertLinks(matches, this.errorMatcher.goto.bind(this.errorMatcher));
    }.bind(this));
  ...
  startNewBuild: function(source, targetName) {
    ...
    Promise.resolve()
      ...
      .then(function (targets) {
        ...
        // tell the process, that we support colored output
