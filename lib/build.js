...
module.exports = {
  ...
  refreshTargets: function() {
    ...
    var bases = [
      OS.homedir() + '/.atom/packages/build',
      this.activePath(),
      OS.tmpdir()
    ];
    ...
    paths.push( bases[0] + '/.atom-build-defaults.json' );
    
    var customPaths = [
      bases[1] + '/.atom-build.json',
      bases[1] + '/.atom-build.cson'
    ]
    
    var usecustom, isjson;
    
    if      (fs.existsSync(customPaths[0])) { paths.push(customPaths[0]); usecustom = true;  isjson = true;  }
    else if (fs.existsSync(customPaths[1])) { paths.push(customPaths[1]); usecustom = true;  isjson = false; }
    else                                    { paths.push(null);           usecustom = false; isjson = true;  }
    
    paths.push( bases[2] + '/.atom-build.json' );
    ...
    if( fs.existsSync(paths[0]) ) {
      filedata.push( fs.readFileSync(paths[0], 'utf8') );
    } else {
      // No defaults to add, so just ignoring the rest
      
      // atom.notifications.addWarning('No atom-build-defaults found!');
      
      // Run custom file
      return this.loadConfig( bases[1] );
    }
    
    if(usecustom) {
      filedata.push( fs.readFileSync(paths[1], 'utf8') );
    } else {
      filedata.push( '{}' );
    }
    ...
    json.push( JSON.parse(filedata[0]) );
    
    if ( isjson ) {
      json.push( JSON.parse(filedata[1]) );
    } else {
      json.push( CSON.parse(filedata[1]) );
    }
    
    // Adding defaults
    
    var custom = json[1];
    var targets = custom['targets'] ? custom['targets'] : {};
    
    for( var key in json[0] ) {
      var val = json[0][key];
      
      if( targets[key] == undefined ) { // Don't overwrite
        targets[key] = val;
      }
    }
    
    // Setting 'main' command
    
    if( custom['name'] == undefined ) {
      var key;
      for(key in targets) break; // Get first element from targets
      
      var target = targets[key];
      delete targets[key];
      
      var defaults = this.cmdDefaults(bases[1]);
      
      custom['name'] = key;
      
      var opts = [ 'cmd', 'args', 'sh', 'cwd', 'env', 'errorMatch', 'keymap' ];
      for( var i in opts ) {
        var k = opts[i];
        custom[k] = target[k] ? target[k] : defaults[k];
      }
    }
    
    custom['targets'] = targets;
    
    fs.writeFileSync( paths[2], JSON.stringify(custom) );
    
    // Run temp file
    return this.loadConfig( bases[2] );
  },
