...
export default {
  ...
  refreshTargets(refreshPaths) {
    ...
    const pathPromise = refreshPaths.map((p) => {
      ...
      this.instancedTools[p] = (this.instancedTools[p] || []).map(t => t.removeAllListeners('refresh')).filter(() => false);
      const settingsPromise = this.tools.map((Tool) => {
        const tool = new Tool(p);
        this.instancedTools[p].push(tool);
        return tool;
      }).filter(tool => tool.isEligible()).map(tool => {
        GoogleAnalytics.sendEvent('build', 'tool eligible', tool.getNiceName());

        tool.on('refresh', this.refreshTargets.bind(this, [ p ]));
  ...
  consumeBuilderLegacy(builders) {
    Grim.deprecate('Use v2 of builder provider instead', { packageName: 'build' });
