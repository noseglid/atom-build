...
module.exports = {
      ...
      .then(function (tools) {
        ...
        return _.chain(settings)
          .map(function(s) {
            ...
            return _.map(s.targets, function (target, name) {
              return [ name, _.defaults(target, this.cmdDefaults(path)) ];
            }.bind(this));
          }.bind(this))
          .compact()
          .flatten()
        ...
        return _.reduce(targets, function(targets, target) {
  ...
  buildCommand: function() {
    ...
    return Promise.resolve(tools)
      ...
      .then(function (tools) {
        ...
        if (!tool) {
          throw new NoToolError('No tool fits the current setup.');
        }

        return tool.settings(path);
      }).then(function (settings) {
        return _.defaults(settings, this.cmdDefaults(path));
      });
  },
  ...
  startNewBuild: function() {
    ...
    Promise.resolve()
      .bind(this)
      .then(function () {
        return this.activeTarget ? this.targets[this.activeTarget] : this.buildCommand();
      })
      .then(function (cmd) {
        this.cmd = cmd;
