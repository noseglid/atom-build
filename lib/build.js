...
export default {
  ...
  startNewBuild(source, targetName) {
    ...
    Promise.resolve(this.targets[p]).then(targets => {
      if (this.linterRegistry !== null) {
        this.linter = this.linterRegistry.register({name: targetName});
      ...
      this.child.on('close', (exitCode) => {
        ...
        for (const error of this.errorMatcher.currentMatch) {
          this.linter.setMessages([{
            ...
            filePath: error.file,
            range: [
              [error.lineStart - 1 || error.line - 1,
                error.columnStart - 1 || error.column - 1],
              [error.lineEnd - 1 || error.line - 1,
                error.columnEnd - 1 || error.column - 1]
            ]
          }]);
        }
        this.child = null;
