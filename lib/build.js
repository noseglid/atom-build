...
module.exports = {
  ...
  activate: function(state) {
    ...
    atom.commands.add('atom-workspace', 'build:build-active-target', function() {
      this.buildActiveTarget();
    }.bind(this));
  ...
  refreshTargets: function() {
    ...
    var targets = _.chain(tools)
      .filter(function(tool) {
        return tool.isEligable(path)
      })
      .map(function(tool) {
        var settings = tool.settings(path);
        if (!settings.targets) { return null; }
        return _.map(settings.targets, function(target, name) {
          return [name, _.defaults(target, cmd)];
        });
      })
      .compact()
      .flatten(true)
    ...
    // turn the array of target descriptions into one object. Targets will be
    // overwritten by later targets with the same name if present.
    targets = _.reduce(targets, function(targets, target) {
      targets[target[0]] = target[1];
      return targets;
    }, {});

    if (!_.isNull(this.activeTarget) && _.isUndefined(targets[this.activeTarget])) {
      // our active target has been removed, so it is no longer valid
      this.activeTarget = null;
    }
  ...
  selectActiveTarget: function() {
    var targets = this.refreshTargets();
    var targetNames = _.keys(targets);
    var targetView = new TargetsView();
    targetView.show(targetNames, this.activeTarget, function(newTarget) {
      this.activeTarget = newTarget;
    }.bind(this));
  },

  buildActiveTarget: function() {
    // TODO: show an error if we have no active target?
    if (_.isNull(this.activeTarget)) { return; }
    this.build(this.activeTarget);
  },
  ...
  startNew      console.log('New active target');
      console.log(this.activeTarget);
 function(targetName) {
    this.cmd = {};
    this.match = [];
    try {
      this.cmd = targetName ? this.targets[targetName] : this.buildCommand();
    } catch (error) {
