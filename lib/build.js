...
module.exports = {
  ...
  buildCommand: function() {
    var cmd = {
      ...
      sh: true,
      errorMatch: '',
      errorMatchPathAbsolute: false
    };
    // Search for a folder in the project folders, that has at a
    // build file in it (at least .atom-build.json).
    _.find(atom.project.getPaths(), function(path) {
        var found = false;
  ...
  errorMatchGoTo: function () {
    atom.workspace.open(!this.cmd.errorMatchPathAbsolute ? this.replace(this.cmd.cwd) + path.sep + this.match[0].file : this.match[0].file, {
