...
export default {
  ...
  selectActiveTarget() {
    ...
    this.targetsView.awaitSelection().then((newTarget) => {
      if (newTarget.variants) {
        this.variantsSubscriptions = atom.commands.add('atom-workspace', Object.keys(newTarget.variants).reduce((commands, variant) => commands[variant] = () => this.build('trigger-' + variant), {}));
      }
