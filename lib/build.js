...
export default {
  ...
  updateStatusBar() {
    ...
    this.statusBarView && this.statusBarView.setTarget(this.targetManager.getActiveTarget(path).name);
  },

  fillTargets() {
    if (this.targetsView) {
      const path = require('./utils').activePath();
      this.targetsView.setActiveTarget(this.targetManager.getActiveTarget(path).name);

      const targetNames = this.targetManager.getTargets(path).map(t => t.name);
      this.targetsView.setItems(targetNames);
    }
  },
  ...
  selectActiveTarget() {
    ...
    if (this.targetManager.isLoading(path)) {
    ...
    this.targetsView.awaitSelection().then(newTarget => {
      this.targetManager.setActiveTarget(path, newTarget);
      this.updateStatusBar();
