...
export default {
  ...
  startNewBuild(source, targetName) {
    ...
    Promise.resolve(this.targets[p]).then(targets => {
      ...
      const errorFile = target.errorFile ? this.replace(target.errorFile, target.env) : null;
      ...
      this.child.on('close', (exitCode) => {
        this.errorMatcher.set(target.errorMatch, cwd, this.buildView.output.text(), errorFile);
