...
export default {
    ...
    const pathPromise = refreshPaths.map((p) => {
      ...
      return Promise.all(settingsPromise).then((settings) => {
        ...
        if (null === this.activeTarget[p] || !settings.find(s => s.name === this.activeTarget[p])) {
          ...
          this.setActiveTargetName(settings[0] ? settings[0].name : undefined, p);
        }
  ...
  setActiveTargetName(targetName, p) {
    ...
    if (targetName) {
      Promise.resolve(this.targets[p]).then(targets => {
        ...
        if (newTarget && newTarget.variants) {
          const commands = Object.keys(newTarget.variants).reduce((table, variant) => {
            const triggerName = 'trigger-' + variant;
            table['build:' + triggerName] = () => this.build(triggerName);
            return table;
          }, {});
          this.variantsSubscriptions = atom.commands.add('atom-workspace', commands);
        }
      });
    }
  },
  ...
  selectActiveTarget() {
    ...
    this.targetsView.awaitSelection().then((newTarget) => {
      this.setActiveTargetName(newTarget, p);
      this.updateStatusBar();
  ...
  startNewBuild(source, atomCommandName) {
    ...
    Promise.resolve(this.targets[p]).then(targets => {
      ...
      if (variant) {
        /* Copy to avoid overwriting properties the original target’s properties with variant properties. */
        target = Object.assign({}, target, target.variants[variant]);
      }
