...
module.exports = (function () {
  ...
  ErrorMatcher.prototype._parse = function (output, htmlOutput) {
    ...
    // the hard part is to find the corresponding places in htmlOutput for the matches
    // we need to be able to deal correctly with cases like the following:
    // ....<b><a>           ...</a>.<c>.            .......           .</c>.<f>.</f>.<d>..            ....</d>...<e>..</e></b>
    //                      ------------                              --------------------
    //                        1. match                                      2. match
    // which should transform to:
    // ....<b><a></a><m1><a>...</a>.<c>.</c></m1><c>.......</c><m2><c>.</c>.<f>.</f>.<d>..</d></m2><d>....</d>...<e>..</e></b>
    this.currentMatch = [];
    ...
    var addOutput = function (destIndex) {
      console.log("to:", destIndex);
      var prevHtmlIndex = htmlIndex;
      var opened = [];
      var closed = [];
      var numOldTags = openTags.length;
      ...
      console.log(prevHtmlIndex, "->", htmlIndex);
      console.log("orig:", escape(output.substr(index, destIndex - index)));
      index = destIndex;
      while (tagIndex < tags.length && tags[tagIndex].index <= htmlIndex)
      {
        ...
        console.log("tag:", tag);
        if (tag.closing) {
          if (openTags.length <= numOldTags) { // < should never occur
            closed.push({
              open: openTags[openTags.length - 1][0],
              close: tag[0]
            });
            numOldTags -= 1;
          }
      ...
      for (var i = numOldTags; i < openTags.length; i++) {
        opened.push({
          open: openTags[i][0],
          close: '</' + openTags[i].tagname + '>'
        });
      }
      return {
        text: htmlOutput.substr(prevHtmlIndex, htmlIndex - prevHtmlIndex),
        opened: opened,
        closed: closed
      };
    };
    for (var matchIndex in this.currentMatch) {
      ...
      // add / close tags before match
      newOutput += addOutput(match.index).text;

      var inner = addOutput(match.index + match[0].length);
      console.log(inner);
      for (let closed of inner.closed) {
        newOutput += closed.close;
      }
      newOutput += util.format('<a class="%s" id="%s">', match.type, match.id);
      for (let closed of inner.closed) {
        newOutput += closed.open;
      }

      newOutput += inner.text;

      for (let opened of inner.opened) {
        newOutput += opened.close;
      }
      newOutput += '</a>';
      for (let opened of inner.opened) {
        newOutput += opened.open;
      }
    }
