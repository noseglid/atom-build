...
module.exports = (function () {
  ...
  ErrorMatcher.prototype._goto = function () {
    var file = this.cwd + path.sep + this.currentMatch[0].file;
    if (!this.currentMatch[0].file) {
    ...
    fs.exists(file, function (exists) {
      ...
      this.emit('scroll', this.currentMatch[0].marker);
  ...
  ErrorMatcher.prototype._parse = function () {
    ...
    for (matchIndex in this.currentMatch) {
      var search = this.currentMatch[matchIndex][0];
      var replace = '<b id=\"' + this.currentMatch[matchIndex].marker + '\">' + this.currentMatch[matchIndex][0] + '</b>';
      output = XRegExp.replace(output, search, replace);
    }
