...
export default class ErrorMatcher extends EventEmitter {
  ...
  goto(id) {
    let match = this.currentMatch.find(m => m.id === id);
    if (!match) {
    ...
    let row = match.line ? match.line - 1 : 0; /* Because atom is zero-based */
    let col =  match.col ? match.col - 1 : 0; /* Because atom is zero-based */
  ...
  _parse() {
    this.currentMatch = [];
    let self = this;
    let matchFunction = function (match, i, string, regex) {
    ...
    this.currentMatch.forEach((match, index) => this.emit('match', match[0], match.id));
  }
