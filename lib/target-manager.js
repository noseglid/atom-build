...
class TargetManager extends EventEmitter {
    ...
    Promise.all(pathPromises).then(entries => {
      ...
      if (atom.config.get('build.notificationOnRefresh')) {
        const rows = refreshPaths.map(path => {
          const count = this.pathTargets.find(pt => pt.path === path).targets.length;
          return `${count} targets at: ${path}`;
        });
  ...
  fillTargets(path) {
    if (this.targetsView) {
      this.targetsView.setActiveTarget(this.getActiveTarget(path).name);
  ...
  getTargets(path) {
    return this.pathTargets.find(pt => pt.path === path).targets;
  }
